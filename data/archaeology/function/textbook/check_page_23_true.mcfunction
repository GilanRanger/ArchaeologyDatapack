# Check function for page 23 (True clicked)
            # Reset tag counter
...
execute if block -193 6 16 minecraft:hopper{Items:[{components:{"minecraft:custom_data":{tag:24}}}]} run scoreboard players add @s tagCheck 1
            ...
            execute if score @s tagCheck matches 2.. if score @s textbook_q23 matches 0 run scoreboard players add @s textbook_correct 1
            execute if score @s tagCheck matches 2.. if score @s textbook_q23 matches 0 run scoreboard players set @s textbook_q23 1
